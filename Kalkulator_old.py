...
def solve_nonlinear():
    ...
    while (b - a) / 2 > e:
        ...
        xc = (a + b) / 2
        fm = f(xc)
        print(f" Krok {krok}: a = {a:.2f}; b = {b:.2f}; środek = {xc:.2f}; f(a) = {f(a):.2f}; f(b) = {f(b):.2f}; f(mid) = {fm:.2f}")
...
def solve_linear_systems():
    ...
    for i in range(min(n, m)):
        ...
        for j in range(i + 1, n):
            factor = matrix[j][i]
            print(f" Eliminacja: odejmujemy {factor:.2f} razy wiersz {i} od wiersza {j}.")
            for k in range(i, m + 1):
    ...
    if n == m:
        ...
        for i in range(n - 1, -1, -1):
            ...
            print(f"\n x{i+1} = {x_res[i]:.2f}")
        print(f"*** Wynik: {', '.join([f'x{i+1} = {x:.2f}' for i, x in enumerate(x_res)])} ***")
    else:
